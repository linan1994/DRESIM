%LP solver for single_repair case.
%consider a repairing team that each step can only repair one
%in this case, we can only use brute force to search for the optimal policy
...
Gnew(i,i)=0;
   ...
   for j=1:2^(sum(Gtotal(:,i)))
       ...
       %fill in cost %not correct here
       %C1{i}(1,j)=C1{i}(1,j)+cost(a);
...
%final one
%cost should appear here
Constraints=Constraints + (E1{N}-0.1*w(N+1)-cost(a)<=0);
...
for i=1:N
    C0{i}=C0{i}*value(w(i));
      ...
      %%  
        ...
        Reward(action)=Reward(action)+cost(action);
